...
namespace DoseWizard
{
    class Program
    {
        ...
        public static void PerformDoseWizardry(Patient patient, PlanIdentification planIdentity)
        {
            ...
            WriteBasicDosimetricData(patient, selectedPlan, structureEsophagus);
            return;
        ...
        private static void WriteBasicDosimetricData(Patient patient, PlanSetup plan, Structure structure)
        {
            ...
            if (!File.Exists(fileName))
            {
                using (StreamWriter writer = File.CreateText(fileName))
                {
                    ...
                    writer.WriteLine("Mean\tMedian\tMaxDose\tVolume");
                }
            }
        ...
        private static void WriteBasicDosimetricData(Patient patient, PlanningItem plan, Structure structure)
        {
            string fileName = "basic dosimetric.txt";
            ...
            DVHData dvhRelative = plan.GetDVHCumulativeData(structure, DoseValuePresentation.Absolute, VolumePresentation.Relative, 0.001);
            if (!File.Exists(fileName))
            {
                using (StreamWriter writer = File.CreateText(fileName))
                {
            ...
            {
                writer.Write(patient.Id + "\t");
                double volumeRelative, volumeAbsolute;
                for (int doseInGray = 10; doseInGray <= 70; doseInGray += 5)
                {
                    DoseValue doseValue = new DoseValue(doseInGray * 100.0, DoseValue.DoseUnit.cGy);
                    volumeAbsolute = FindVolumeAtDose(dvhAbsolute, structure, doseValue);
                    writer.Write("{0:0.00}\t", volumeAbsolute);
                }
                for (int doseInGray = 10; doseInGray <= 70; doseInGray += 5)
                {
                    DoseValue doseValue = new DoseValue(doseInGray * 100.0, DoseValue.DoseUnit.cGy);
                    volumeRelative = FindVolumeAtDose(dvhRelative, structure, doseValue);
